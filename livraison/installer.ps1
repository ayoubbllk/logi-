...
if ($mysqlMode -eq "none") {
  ...
  if ($mysqlService) {
    # Extraire le dossier bin du service
    $svcPath = $mysqlService.PathName -replace '"', ''
    $svcBinDir = Split-Path $svcPath -Parent
    $localMysqlExe = Join-Path $svcBinDir "mysql.exe"
  ...
  if ($mysqlMode -eq "none" -and (Test-Command "mysql")) {
    ...
    # Verifier qu'un serveur MySQL ecoute sur le port
...
if ($mysqlMode -eq "docker") {
  ...
  $dbUrlForEnv = "mysql://${dbUser}:${dbPass}@localhost:${mysqlPort}/${dbName}"
}
...
if ($mysqlMode -eq "docker") {
  ...
  do {
    ...
    $retry++
    try {
      $result = docker exec qc-db mysqladmin ping -h localhost -u qcuser -pqc_secure_password 2>&1
  ...
  if ($mysqlReady) {
    ...
    Write-Warn "MySQL n'a pas repondu dans le delai. Verifiez avec 'docker ps'"
  }
  ...
  # Demander le mot de passe root si besoin
  $testResult = & $mysqlBin -u root --port=$mysqlPort -e "SELECT 1" 2>&1
  ...
  & $mysqlBin -u root -p"$dbPass" --port=$mysqlPort -e "CREATE DATABASE IF NOT EXISTS ``$dbName`` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" 2>&1 | Out-Null
  if ($LASTEXITCODE -ne 0) {
  ...
  # Mettre a jour le .env avec le bon mot de passe
  $newDbUrl = "mysql://${dbUser}:${dbPass}@localhost:${mysqlPort}/${dbName}"
